...
Feature: The my overview with search block allows users to easily access their courses
  In order to enable the my overview with search block in a course
  As a student
  I can add the my overview with search block to my dashboard
  ...
  Scenario: View all (including removed) courses
    Given the following config values are set as admin:
      | config                            | value | plugin           |
      | displaygroupingallincludinghidden | 1     | block_myoverview_withsearch |
  ...
  Scenario: Show course category in cards display
    Given the following config values are set as admin:
      | displaycategories | 1 | block_myoverview_withsearch |
    And I log in as "student1"
    And I click on "Display drop-down menu" "button" in the "Course overview" "block"
  ...
  Scenario: Hide course category in cards display
    Given the following config values are set as admin:
      | displaycategories | 0 | block_myoverview_withsearch |
    And I log in as "student1"
    And I click on "Display drop-down menu" "button" in the "Course overview" "block"
    When I click on "Card" "link" in the "Course overview" "block"
